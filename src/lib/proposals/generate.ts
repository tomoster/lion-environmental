...
export async function generateXRFProposal(data: ProposalData): Promise<Buffer> {
  ...
  y = drawTable(doc, y, [
    ...
    { cells: [{ text: "New York State Tax\n8.8%", bold: true }, { text: "" }, { text: "" }, { text: subtotal > 0 ? fmt(tax) : "" }], height: 30 },
    { cells: [{ text: "TOTAL", bold: true }, { text: "" }, { text: "" }, { text: subtotal > 0 ? fmt(total) : "", bold: true }], height: 30 },
  ], colW);
...
export async function generateDustSwabsProposal(data: ProposalData): Promise<Buffer> {
  ...
  y = drawTable(doc, y, [
    ...
    { cells: [{ text: "New York State Tax\n8.8%", bold: true }, { text: "" }, { text: "" }, { text: subtotal > 0 ? fmt(tax) : "" }], height: 30 },
    { cells: [{ text: "TOTAL", bold: true }, { text: "" }, { text: "" }, { text: subtotal > 0 ? fmt(total) : "", bold: true }], height: 28 },
  ], colW);
